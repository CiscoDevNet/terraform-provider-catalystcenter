...
- Fix `catalystcenter_assign_credentials` resource so credentials can be unassigned and cleanly removed without affecting credentials inherited from a parent site. On destroy, and on in-place updates that clear a slot, the provider now issues a PUT that unassigns only the credential slots Terraform manages (those that are non-null in state) by sending `null` for each; per the API a null value makes the slot inherit from the parent site, while slots that were never set are omitted so other inherited credentials (or credentials managed by another assignment) are left untouched. Previously the destroy always cleared all six slots, which wiped inherited credentials at child sites. Unassigning the credential lets the referenced `catalystcenter_credentials_*` object be deleted instead of failing with `NCIM01100`. The global site has no parent to inherit from and rejects a partial payload with `NCND01090`; in that case the destroy retries once with all six slots present as empty objects (`{}`, the API's "unset" form). Empty-string credential references (`objReferences: [""]`, returned by Catalyst Center for HTTP read/write and other unset/inherited slots) are now read as null, eliminating a permanent in-place diff on every plan/refresh. Transient `NCND00010` ("Global Settings Save is in progress") errors are retried on create, update, and delete (15s interval, up to the provider's maximum async wait time).
