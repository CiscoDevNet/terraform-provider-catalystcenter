...
- Add `catalystcenter_access_point_configuration` resource to configure one or more access points through the Catalyst Center Configure Access Points V1 intent API (`/dna/intent/api/v1/wireless/accesspoint-configuration`), including renaming an access point via `ap_name_new`. Access points are selected by their ethernet MAC address, and a single resource configures a list of access points sharing the same settings. Each value is applied only when its matching `configure_*` flag is set. The endpoint is an asynchronous action API with no corresponding GET, so the resource is create-only: it cannot be read back, refreshed or imported, a change to any attribute forces recreation, and destroying it does not revert any configuration on the access points
