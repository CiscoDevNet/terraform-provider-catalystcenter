...
func TestAccCcCredentialsSNMPv2Write(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccCcCredentialsSNMPv2WriteConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccCcCredentialsSNMPv2WriteConfig_all(),
		Check:  resource.ComposeTestCheckFunc(checks...),
	})
	steps = append(steps, resource.TestStep{
		ResourceName: "catalystcenter_credentials_snmpv2_write.test",
		ImportState:  true,
	})
