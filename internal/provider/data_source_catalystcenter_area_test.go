...
func TestAccDataSourceCcArea(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.catalystcenter_area.test", "name", "Area1"))
...
func testAccDataSourceCcAreaConfig() string {
	...
	config += `
		data "catalystcenter_area" "test" {
			id = catalystcenter_area.test.id
		}
	`
	return config
}
