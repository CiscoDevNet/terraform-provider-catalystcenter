...
func TestAccDataSourceCcBuildings(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.catalystcenter_buildings.test", "buildings.0.latitude", "37.338"))
	checks = append(checks, resource.TestCheckResourceAttr("data.catalystcenter_buildings.test", "buildings.0.longitude", "-121.832"))
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceCcBuildingsPrerequisitesConfig + testAccDataSourceCcBuildingsConfig(),
...
func testAccDataSourceCcBuildingsConfig() string {
	...
	config += `
		data "catalystcenter_buildings" "test" {
			id = catalystcenter_buildings.test.id
		}
	`
	return config
}
