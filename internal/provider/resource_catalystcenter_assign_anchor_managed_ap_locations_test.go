...
func TestAccCcAssignAnchorManagedAPLocations(t *testing.T) {
	if os.Getenv("WIRELESS") == "" {
		t.Skip("skipping test, set environment variable WIRELESS")
	}
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("catalystcenter_assign_anchor_managed_ap_locations.test", "device_id", "75b0f85a-8157-4db3-ae2d-9807c893319a"))

	var steps []resource.TestStep
	steps = append(steps, resource.TestStep{
		Config: testAccCcAssignAnchorManagedAPLocationsPrerequisitesConfig + testAccCcAssignAnchorManagedAPLocationsConfig_all(),
...
const testAccCcAssignAnchorManagedAPLocationsPrerequisitesConfig = `
...
func testAccCcAssignAnchorManagedAPLocationsConfig_minimum() string {
	config := `resource "catalystcenter_assign_anchor_managed_ap_locations" "test" {` + "\n"
	config += `	anchor_managed_ap_locations_site_ids = [catalystcenter_building.test.id]` + "\n"
...
func testAccCcAssignAnchorManagedAPLocationsConfig_all() string {
	config := `resource "catalystcenter_assign_anchor_managed_ap_locations" "test" {` + "\n"
	config += `	anchor_managed_ap_locations_site_ids = [catalystcenter_building.test.id]` + "\n"
	config += `	device_id = "75b0f85a-8157-4db3-ae2d-9807c893319a"` + "\n"
	config += `}` + "\n"
