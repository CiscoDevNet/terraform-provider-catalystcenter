...
// wiped) and sends each cleared slot as a top-level null (the API's "inherit
// from parent" form).
...
// assertNullSlot asserts the slot is present as a top-level null (the API's
// "inherit from the parent site" form), e.g. {"cliCredentialsId": null}.
...
// On Update that clears a single managed slot, only that slot is sent as a
// top-level null (inherit); the still-configured slot keeps its value and
// never-set (inherited) slots stay omitted.
...
// Terraform manages (non-null in state), each as a top-level null (inherit);
// inherited slots are omitted so they are preserved.
