...
func (d *CredentialsHTTPSReadDataSource) ConfigValidators(ctx context.Context) []datasource.ConfigValidator {
...
func (d *CredentialsHTTPSReadDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if config.Id.IsNull() && !config.Description.IsNull() {
		...
		if value := res.Get("response.httpsRead"); len(value.Array()) > 0 {
