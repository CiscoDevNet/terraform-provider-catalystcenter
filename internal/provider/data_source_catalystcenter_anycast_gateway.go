...
func (d *AnycastGatewayDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	params := ""
	params += "?fabricId=" + url.QueryEscape(config.FabricId.ValueString()) + "&virtualNetworkName=" + url.QueryEscape(config.VirtualNetworkName.ValueString()) + "&ipPoolName=" + url.QueryEscape(config.IpPoolName.ValueString())
