...
func TestAccCcCredentialsSNMPv2Read(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccCcCredentialsSNMPv2ReadConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccCcCredentialsSNMPv2ReadConfig_all(),
		Check:  resource.ComposeTestCheckFunc(checks...),
	})
	steps = append(steps, resource.TestStep{
		ResourceName: "catalystcenter_credentials_snmpv2_read.test",
		ImportState:  true,
	})
