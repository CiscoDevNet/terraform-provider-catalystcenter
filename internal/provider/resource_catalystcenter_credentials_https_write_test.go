...
func TestAccCcCredentialsHTTPSWrite(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccCcCredentialsHTTPSWriteConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccCcCredentialsHTTPSWriteConfig_all(),
		Check:  resource.ComposeTestCheckFunc(checks...),
	})
	steps = append(steps, resource.TestStep{
		ResourceName: "catalystcenter_credentials_https_write.test",
		ImportState:  true,
	})
...
func testAccCcCredentialsHTTPSWriteConfig_minimum() string {
...
func testAccCcCredentialsHTTPSWriteConfig_all() string {
	config := `resource "catalystcenter_credentials_https_write" "test" {` + "\n"
	config += `	description = "My HTTPS write credentials"` + "\n"
	config += `	username = "user1"` + "\n"
	config += `	password = "password1"` + "\n"
